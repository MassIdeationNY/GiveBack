var Kalender = function(){
  ...
  this.bookCalender = function( booked, callback){ console.log(" kalendar.bookCalendar - add new dates...") }
  this.clearCalender = function(div, booked, startDate, callback){ console.log("kalendar.bookCalendar - clear our data...") }
  ...
  this.insertCalender = function(div, booked, startDate, callback){
    ...
    this.addSelectedDate = function(d){
    ...
    function onDateClicked(evt, bookedDates ){
      console.log("kalendar: date clicked: " + evt.target );
    ...
    function dateIsBlocked(dateToCheck)
     ...
     function dateIsSelected(dateToCheck)
     {
      var selected = false;
      for (var i=0; i<selected_dates.length; i++){
        var selectedDate = selected_dates[i];
        var isSameDay = (dateToCheck.getDate() == selectedDate.getDate() 
    ...
    function changeMonth(m,year){
      ...
      for (var week_num=0; week_num<=5; week_num++){
        for (var day_index=0; day_index<7; day_index++) {
          ...
          var cell_date = new Date(current_year_value, current_month_value, current_date+1);

          cell.removeClass("day-none day-blocked day-selected day");



          if (dateIsBlocked(cell_date)){
            cell.addClass("day-blocked");
          }else if(dateIsSelected(cell_date)){
